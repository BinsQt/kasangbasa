        ...
        3: {
            ...
            slides: [
                ...
                },
                {
                    id: 3,
                    layoutType: 'LAYOUT_3', // Add layoutType
                    title: "Hakbang Tungo sa Pag-unlad",
                    content: [
...
// Assuming slidesData, SlideTemplate, convertTextToSpeech, speakLocal, and stopAudio are imported from elsewhere
// import { slidesData } from './data'; // Example import
// import SlideTemplate from './SlideTemplate'; // Example import
// import { convertTextToSpeech, speakLocal, stopAudio } from './audioService'; // Example import
...
const Module = () => {
  ...
  // Parse URL parameters with clear defaults
  const subtopicIdFromUrl = searchParams.get('subtopicId');
  const autoplayFromUrl = searchParams.get('autoplay') === 'true';
  const voiceOverFromUrl = searchParams.get('voiceOver') === 'true';

  // State management for UI and audio controls
  const [currentSubtopic, setCurrentSubtopic] = useState(
    subtopicIdFromUrl ? Number(subtopicIdFromUrl) : null
  );
  ...
  // Memoize main topic and subtopics for performance
  const mainTopic = useMemo(() => slidesData[id] || {}, [id]);
  const subtopics = useMemo(() => mainTopic.subtopics || {}, [mainTopic]);

  // Memoize current slides based on selected subtopic
  const slides = useMemo(
    () => (currentSubtopic ? subtopics[currentSubtopic]?.slides || [] : []),
    [currentSubtopic, subtopics]
  );
  const slide = useMemo(
    () => slides.find((s) => s.id === currentSlide) || null,
    [slides, currentSlide]
  );

  // Determine the title to display
  const displayedTitle = useMemo(
    () => (currentSubtopic ? subtopics[currentSubtopic]?.title : mainTopic.title) || 'Module Overview',
    [currentSubtopic, subtopics, mainTopic.title]
  );
  ...
  // Memoized function to stop audio and reset pause state
  ...
  // Function to play text-to-speech for a given content
  const playSlideAudio = useCallback(
    (contentArray) => {
      if (!voiceOverFromUrl || isMuted || !contentArray || contentArray.length === 0) {
        return;
      }
      const slideContent = contentArray.join(' ');
      if (typeof convertTextToSpeech === 'function') {
        convertTextToSpeech(slideContent).catch(() => {
          if (typeof speakLocal === 'function') {
            speakLocal(slideContent);
          }
        });
      } else if (typeof speakLocal === 'function') {
        speakLocal(slideContent);
      }
      setIsPaused(false);
    },
    [voiceOverFromUrl, isMuted]
  );

  // --- Effect Hooks ---

  // Effect to reset slide and subtopic on module or subtopic change
  useEffect(() => {
    setCurrentSlide(1);
    setCurrentSubtopic(subtopicIdFromUrl ? Number(subtopicIdFromUrl) : null);
    handleStopAudio(); // Ensure audio stops when navigating to a new module/subtopic
  }, [id, subtopicIdFromUrl, handleStopAudio]);

  // Effect to stop audio on location change (e.g., if navigating away)
  useEffect(() => {
    handleStopAudio();
  }, [location, handleStopAudio]);

  // Autoplay effect for voice-over
  useEffect(() => {
    // Only autoplay if voiceOver is enabled, autoplay is true, and it's not module '3'
    if (voiceOverFromUrl && autoplayFromUrl && id !== '3' && slide && !isMuted) {
      const delay = setTimeout(() => {
        playSlideAudio(slide.content);
      }, 500); // Small delay to ensure render before audio starts

      return () => clearTimeout(delay);
    } else {
      // Ensure audio is stopped if conditions for autoplay aren't met
      handleStopAudio();
    }
  }, [voiceOverFromUrl, autoplayFromUrl, slide, id, isMuted, handleStopAudio, playSlideAudio]);

  // --- Handlers ---

  const handleSubtopicSelect = (selectedSubtopicId) => {
    handleStopAudio();
    setCurrentSubtopic(selectedSubtopicId);
    setShowSubtopicList(false);
    setCurrentSlide(1); // Always go to the first slide of the new subtopic
  };

  const handleSlideChange = (newSlideNumber) => {
    if (newSlideNumber < 1 || newSlideNumber > slides.length) {
      console.warn('Attempted to navigate to an invalid slide number:', newSlideNumber);
      return;
    }

    handleStopAudio(); // Stop current audio before changing slides
    setCurrentSlide(newSlideNumber);

    // If voiceOver is active and not muted, play audio for the new slide
    if (voiceOverFromUrl && id !== '2' && !isMuted) {
      const newSlideContent = slides[newSlideNumber - 1];
      if (newSlideContent) {
        playSlideAudio(newSlideContent.content);
      }
    }
  };
  ...
  const handlePauseAudio = () => {
    handleStopAudio(); // This effectively pauses by stopping current playback
    setIsPaused(true);
  };
  ...
  const toggleMute = () => {
    ...
    if (!isMuted) {
      handleStopAudio(); // Stop audio immediately if muting
    }
  };
  ...
  return (
    <div className="container mx-auto p-4 bg-gray-50 min-h-screen relative">
      {/* Header Section */}
      <div className="flex flex-col sm:flex-row justify-between items-center py-4 px-2 bg-white rounded-lg shadow-md mb-6">
        <button
          onClick={() => setShowSubtopicList(!showSubtopicList)}
          className="bg-green-600 hover:bg-green-700 text-white p-3 rounded-lg flex items-center justify-center transition-colors duration-200 mb-4 sm:mb-0"
          aria-label="Toggle Subtopic List"
        >
          <svg
          ...
          <span className="ml-2 hidden sm:inline">Subtopics</span>
        </button>
        <h1 className="text-center text-3xl font-extrabold text-gray-800 flex-grow">
          {displayedTitle}
        </h1>
        {/* Placeholder for potential right-aligned elements */}
        <div className="w-16 sm:w-auto"></div>
      </div>

      {/* Subtopic List Overlay */}
      {showSubtopicList && (
        <div className="absolute z-10 w-full md:w-1/3 lg:w-1/4 bg-white border border-gray-200 rounded-lg shadow-xl mt-2 p-4 animate-fade-in-down">
          <ul className="space-y-2">
            {Object.entries(subtopics).map(([subId, subtopic]) => (
              <li
                key={subId}
                onClick={() => handleSubtopicSelect(Number(subId))}
                className="cursor-pointer p-3 rounded-md hover:bg-blue-100 text-gray-700 font-medium transition-colors duration-150 flex items-center"
              >
                <span className="text-blue-500 mr-2">&#8227;</span> {subtopic.title}
              </li>
            ))}
          </ul>
          <button
            onClick={() => setShowSubtopicList(false)}
            className="mt-4 w-full bg-red-500 hover:bg-red-600 text-white p-2 rounded-md transition-colors duration-200"
          >
            Close
          </button>
        </div>
      ...
      {/* Main Content Area: Only show if a subtopic is selected */}
      {currentSubtopic ? (
        <SlideTemplate backgroundImage={backgroundImage} voiceOver={voiceOverFromUrl}>
          <div className="flex flex-col h-full">
            {/* Slide Content - Visible on mobile first, then hidden on larger screens where it moves to buttons section */}
            <div className="block sm:hidden flex-grow overflow-auto p-4 bg-white bg-opacity-80 rounded-lg shadow-inner mb-4">
              {slide ? (
                <SlideContent slide={slide} readAloud={speakLocal} />
              ) : (
                <p className="text-gray-600 text-center">Select a subtopic to view slides.</p>
              )}
            </div>

            {/* Navigation and Slide Content for larger screens */}
            <div className="flex flex-col sm:flex-row justify-between items-stretch gap-4 py-4 px-2">
              <button
                onClick={handlePreviousSlide}
                className="bg-gray-400 hover:bg-gray-500 text-white p-3 rounded-lg shadow-md flex-shrink-0 transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed text-lg"
                disabled={currentSlide === 1}
              >
                Previous
              </button>

              {/* Slide Content for larger screens */}
              <div className="hidden sm:block flex-grow bg-white bg-opacity-80 rounded-lg shadow-inner p-6 overflow-auto max-h-[calc(100vh-250px)]">
                {slide ? (
                  <SlideContent slide={slide} readAloud={speakLocal} />
                ) : (
                  <p className="text-gray-600 text-center">Select a subtopic to view slides.</p>
              ...
              <button
                ...
                className="bg-blue-600 hover:bg-blue-700 text-white p-3 rounded-lg shadow-md flex-shrink-0 transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed text-lg"
        ...
        <div className="text-center p-8 bg-white rounded-lg shadow-md mt-6">
          <p className="text-xl text-gray-700">
            Please select a subtopic from the menu to start your learning journey!
          </p>
        </div>
      )}

      {/* Audio Control Buttons (Conditional Rendering) */}
      {id !== '3' && (
        <div className="flex flex-wrap justify-center items-center gap-4 mt-8 p-4 bg-white rounded-lg shadow-md">
          <button
            onClick={handlePlayAudio}
            className={`p-3 rounded-lg shadow-md transition-all duration-200 text-lg font-semibold ${
              !isPaused && slide && !isMuted ? 'bg-green-500 text-white' : 'bg-gray-300 text-gray-700'
            }`}
            disabled={!slide || (!isPaused && !isMuted)} // Disable if no slide or already playing and not muted
          ...
          <button
            ...
            className={`p-3 rounded-lg shadow-md transition-all duration-200 text-lg font-semibold ${
              isPaused || isMuted ? 'bg-gray-300 text-gray-700' : 'bg-red-500 text-white'
            }`}
            disabled={isPaused || isMuted} // Disable if already paused or muted
